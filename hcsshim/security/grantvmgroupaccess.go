...
func GrantVmGroupAccess(name, userSID string) error { //nolint:stylecheck // ST1003
	return GrantVmGroupAccessWithMask(name, userSID, accessMaskDesiredPermission)
}

// GrantVmGroupAccessWithMask sets the desired DACL for a specified file or
// directory.
func GrantVmGroupAccessWithMask(name, userSID string, access accessMask) error { //nolint:stylecheck // ST1003
	...
	newDACL, err := generateDACLWithAcesAdded(name, userSID, s.IsDir(), access, origDACL)
	if err != nil {
		return err // Already wrapped
	}
	defer func() {
...
func generateDACLWithAcesAdded(name, userSID string, isDir bool, desiredAccess accessMask, origDACL uintptr) (uintptr, error) {
